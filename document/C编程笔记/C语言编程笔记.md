...
路径：D:\Keil MDK\ARM\ARMCC\include

数据类型：

![image-20260202195848878](image/image-20260202195848878.png) 
...
![image-20260202200544702](image/image-20260202200544702.png) 

* 左移运算符(<<)：将二进制位向左移动指定的位数，右侧用0填充。左移相当于对数值进行**乘以2的n次方**的操作。左移操作不会改变符号位，因此正数和负数的左移结果都保持符号不变。

* 右移运算符(>>)：将二进制位向右移动指定的位数，左侧的填充方式取决于操作数是否为有符号数：

  * **算术右移**：右移时，左侧用 **符号位(最高位)** 的值填充，以此保留原数的符号（适用于有符号数）。
    对有符号数来说，算术右移等价于对数值进行**除以 2 的n次方并向下取整**的操作。
    ```c
    int a = -5;         // 原码：1000 0101，补码：1111 1011
    int b = a >> 1; // 算术右移1位，结果为-3，补码：1111 1101
    ```
    
    右移时，最高位的符号位1会填充左侧空位，得到新的补码：1111 1101，对应的十进制值为-3，符合 “除以 2 并向下取整” 的效果。
  * **逻辑右移**：左侧用0填充（适用于无符号数）。

    对无符号数来说，逻辑右移相当于对数值进行**除以2的n次方**的操作。
    
    ```c
    unsigned int a = 5; // 二进制为：0000 0101
    unsigned int b = a >> 1; // 逻辑右移1位
    ```
    
    右移后得到`0000 0010`，对应的十进制值为`2`，正好是`5 / 2`的结果。

    **补数原则**

    在C语言中，整数的存储和运算基于**补码**，使得加减运算可以统一处理，同时简化了硬件设计。

    * **原码**：最高位为符号位（`0`表示正数，`1`表示负数），其余位为数值位，直接表示数值的二进制形式。
      （以 8 位二进制为例）：
      +5`的原码：`0000 0101
      -5 `的原码：`1000 0101
    * **反码**：正数的反码与原码相同；负数的反码是对原码的**数值位按位取反**（符号位保持不变）。
      （以 8 位二进制为例）：
      `+5`的反码：`0000 0101`（与原码相同）
      `-5`的反码：`1111 1010`（原码`1000 0101`的数值位取反）
    * **补码**：正数的补码与原码相同；负数的补码是其**反码加 1**。
      （以 8 位二进制为例）：
      `+5`的补码：`0000 0101`（与原码相同）
      `-5`的补码：`1111 1011`（反码`1111 1010`加 1）
    * **补码的核心作用**：让有符号数的加法和减法可以统一为加法运算，无需单独处理减法逻辑。
      例如计算`3 - 5`，在补码中就是`3 + (-5)`：
      3`  的补码：`0000 0011
      -5`的补码：`1111 1011
      相加结果：`1111 1110`（补码），对应的原码为`1000 0010`，即十进制的`-2`，与实际计算结果一致。


* 注意事项

  1. **移位次数限制**：移位次数不能超过变量的位宽，否则结果不可预测。
  2. **符号位处理**：算术右移会保留符号位，而逻辑右移不会。
  3. **负数右移**：不同编译器对负数右移的实现可能不同，应避免依赖未定义行为。
  
* 实际应用

  1. **快速乘除法**：左移相当于乘以2的幂，右移相当于除以2的幂。
  2. **位操作**：结合按位与、或、异或等操作，移位可用于设置、清除或翻转特定位。
  3. **性能优化**：在图像处理、信号处理等领域，移位操作能显著提高效率。
